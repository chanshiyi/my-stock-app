...
# 增加一个监控维度的切换按钮
monitor_type = st.sidebar.selectbox("选择监控维度", ["概念板块资金流", "个股资金流"])
...
# 使用带状态提示的函数
def get_stock_data(data_type):
    with st.spinner("🔄 正在努力请求东方财富数据接口..."):
        try:
            if data_type == "概念板块资金流":
                # 【修正核心】：最新 AkShare 的概念板块资金流向接口
                df = ak.stock_fund_flow_concept_em()
            ...
            st.error("📡 网络连接超时！云端服务器连接国内节点失败，请刷新重试。")
            return pd.DataFrame()
        except Exception as e:
            st.error(f"⚠️ 发生错误：{str(e)}\n\n如果提示找不到函数，请检查 GitHub 上的 requirements.txt 是否包含了最新的 akshare。")
            return pd.DataFrame()

# --- 主面板数据渲染 ---
data_df = get_stock_data(monitor_type)

if not data_df.empty:
    total_inflow = data_df["主力净流入(亿元)"].sum().round(2)
    name_col = "板块名称" if monitor_type == "概念板块资金流" else "简称"
